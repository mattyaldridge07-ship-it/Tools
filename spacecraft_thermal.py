...
if __name__ == '__main__':
    ...
    parser.add_argument('--microsat', action='store_true',
                        help='MicroSat-100 class at 650 km')
    parser.add_argument('--point',   action='store_true',
                        help='Print summary tables only')
    args = parser.parse_args()

    run_cubesat = args.cubesat or not any([args.cubesat, args.microsat, args.point])
    run_msat    = args.microsat or not any([args.cubesat, args.microsat, args.point])

    if args.point:
        print_table('cubesat')
        print_table('microsat')
    else:
        if run_cubesat:
        ...
        if run_msat:
            print("\nSimulating MicroSat-100 class...")
            print_table('microsat')
            plot_analysis('microsat', 'spacecraft_microsat.png')
