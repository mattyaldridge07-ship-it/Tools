"""
linear aerospike rocket nozzle conjugate heat transfer and regenerative cooling channel sizing calculator
"""
...
T_MAT_LIMIT_K  = 1100.0     # max safe steady-state operating temperature
...
def gas_properties_along_spike(x_m):
    """
    local gas conditions along the spike, 1D isentropic expansion approximation
    Mach rises linearly from 1.0 (throat, x=0) to 3.0 (tip, x=0.15)
    """
...
def bartz_heat_transfer_coefficient(M, T_gas_K, T_w_K, A_ratio):
    """Bartz equation, quick estimate of rocket nozzle convective heat transfer coefficient"""
    R_c = 0.05  # m  throat curvature radius approximation

    mu_gas = 4.5e-5  # Pa·s, Sutherland-type power law for combustion products
...
def solve_conjugate_heat_transfer(m_dot_cool_kgs, w_c_mm=1.5, h_c_mm=3.0, N_channels=24):
    """
    marches along the spike computing gas properties, external heat flux, Inconel wall
    ...
    # annular sector approximation: total heated wall width across Nc channels
    ...
    for x in x_vec:
        ...
        # iterate wall temp: Bartz depends on T_w non-linearly
        T_hot_temp = 900.0
        for _ in range(5):
            h_g, T_aw_K = bartz_heat_transfer_coefficient(M, T_gas_K, T_hot_temp, A_ratio)
            # 1D steady conduction: q = U*(T_aw - T_cool), 1/U = 1/h_g + t_wall/k_wall + 1/h_cool
        ...
        # coolant bulk temp rise: dT = q_ext * (perimeter*dx) / (m_dot * Cp)
        dT_cool = (q_ext * perimeter * dx) / (m_dot_cool_kgs * CP_KEROSENE)
        T_cool_K += dT_cool

        f_friction = 0.316 * Re_cool**(-0.25) if Re_cool > 4000.0 else 64.0 / Re_cool  # Blasius
...
def plot_all(output_path):
    ...
    # panel 5: channel width sweep vs max wall T
