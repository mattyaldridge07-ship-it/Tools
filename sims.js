...
function divertorTempColor(T) {
  const t = Math.max(0, Math.min(1, (T - 60) / 840));
  return t < 0.5 ? lerpColor([40, 50, 70], [200, 90, 30], t / 0.5)
  ...
  if (activeTab === 'scramjet') {
    ...
    for (let i = 0; i < 30; i++) {
      divertorState.fluidParticles.push({
        ...
        y: 10 + Math.random() * (canvas.height * 0.75 - 20),
    ...
    for (let i = 0; i < 80; i++) {
      const sx = Math.random() * canvas.width * 0.58;
      const sy = Math.random() * canvas.height * 0.75;
      airfoilState.particles.push({ x: sx, y: sy, px: sx, py: sy, speedMag: 0 });
    }
  }
}
...
function updatePhysics() {
  if (activeTab === 'scramjet') {
    ...
    shockState.particles.forEach(p => {
      const wX = canvas.width * 0.58 * 0.4;
      const wY = canvas.height * 0.38;
    ...
    divertorState.fluidParticles.forEach(p => {
      ...
      if (p.y > canvas.height * 0.75 - 10) p.y = 10;
    });
    ...
    divertorState.bubbles.forEach((b, idx) => {
      ...
      if (b.y > canvas.height * 0.75 - 10 || b.size > 20) {
    ...
    airfoilState.particles.forEach(p => {
      ...
      p.speedMag = Math.sqrt(vel.u * vel.u + vel.v * vel.v);
      p.px = p.x;
      p.py = p.y;
...
function draw() {
  ...
  ctx.fillRect(0, 0, divideX, canvas.height);
...
function drawScramjetSim(width) {
  ...
  gasGrad.addColorStop(0.55, 'rgba(255, 220, 80, 0.5)');
  gasGrad.addColorStop(1.0, 'rgba(140, 100, 100, 0.28)');
  ...
  streakLayers.forEach((layer, idx) => {
    ...
    for (let x = 0; x < width; x += 8) {
      const yOff = Math.sin(x * layer.freq - time * layer.speed + idx * 2) * layer.amp
                 + Math.cos(x * (layer.freq * 0.5) + time * (layer.speed * 0.5)) * (layer.amp * 0.5);
  ...
  scramjetState.fluidParticles.forEach(p => {
    ...
    const heatFrac = Math.min(1, Math.max(0, (localT - 40) / 260));
    const c = lerpColor([64, 176, 192], [208, 75, 75], heatFrac);
    ctx.fillStyle = rgbaStr(c, 1);
    ctx.shadowBlur = 3 + heatFrac * 6;
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const T_cool = scramjetState.T_wall_cool[i] || 350.0;
    const hotColor = `hsl(${Math.max(0, 240 - (T_hot - 300) * 0.35)}, 80%, 45%)`;
    const coolColor = `hsl(${Math.max(0, 240 - (T_cool - 300) * 0.35)}, 80%, 38%)`;

    let topGrad = ctx.createLinearGradient(0, topWallY, 0, topWallY + wallH);
    topGrad.addColorStop(0, coolColor);
    topGrad.addColorStop(1, hotColor);
    ctx.fillStyle = topGrad;
    ctx.fillRect(xStart, topWallY, w, wallH);

    let bottomGrad = ctx.createLinearGradient(0, bottomWallY, 0, bottomWallY + wallH);
    bottomGrad.addColorStop(0, hotColor);
    bottomGrad.addColorStop(1, coolColor);
...
function drawShockSim(width) {
  ...
  ctx.strokeStyle = 'rgba(90, 85, 60, 0.35)';
  ctx.lineWidth = 1;
  const bTop = Math.min(topY, wY) - 10;
  const bBottom = Math.max(bottomY, wY) + 10;
  const bLeft = wX - 10;
  const bRight = topX + 10;
  const span = (bRight - bLeft) + (bBottom - bTop);
  for (let d = -40; d < span + 40; d += 6) {
    ctx.beginPath();
    ctx.moveTo(bLeft + d, bTop);
    ctx.lineTo(bLeft + d - (bBottom - bTop), bBottom);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(bLeft + d, bTop);
    ctx.lineTo(bLeft + d + (bBottom - bTop), bBottom);
  ...
  ctx.strokeStyle = 'rgba(184, 146, 10, 0.55)';
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.moveTo(wX, wY);
  ctx.lineTo(topX, topY);
...
function drawBrakeSim(width) {
  ...
  const innerR = 40;
  const hubR = 18;
  const capAngle = (55 * Math.PI) / 180;
  const capStart = -Math.PI / 2;
  const capEnd = capStart + capAngle;

  const heatT = Math.max(0, Math.min(1, (brakeState.temp - 300) / 500));
  ...
  if (heatT > 0) {
    ...
    ctx.save();
    ctx.shadowBlur = 6 + heatT * 26;
    ctx.shadowColor = rgbaStr(peakC, Math.min(1, 0.5 + heatT * 0.5));

    if (ctx.createConicGradient) {
      let heatGrad = ctx.createConicGradient(contactAngleLocal, 0, 0);
      heatGrad.addColorStop(0, rgbaStr(peakC, 0.15 + heatT * 0.55));
      heatGrad.addColorStop(0.5, rgbaStr([60, 30, 25], 0.05 * heatT));
      heatGrad.addColorStop(1, rgbaStr(peakC, 0.15 + heatT * 0.55));
      ctx.fillStyle = heatGrad;
    } else {
      let heatGrad = ctx.createRadialGradient(0, 0, innerR, 0, 0, outerR);
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const hr = innerR + 6 + (i % 3) * 6;
    ctx.beginPath();
    ctx.arc(Math.cos(a) * hr, Math.sin(a) * hr, 1.6, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  for (let i = 0; i < nBolts; i++) {
    ...
    ctx.beginPath();
    ctx.arc(Math.cos(a) * (hubR * 0.65), Math.sin(a) * (hubR * 0.65), 2.5, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  ctx.translate(cX, cY);
  ctx.beginPath();
  ctx.arc(0, 0, outerR + 10, capStart, capEnd, false);
  ctx.lineTo(Math.cos(capEnd) * (innerR + 10), Math.sin(capEnd) * (innerR + 10));
  ctx.arc(0, 0, innerR + 10, capEnd, capStart, true);
  ctx.closePath();

  if (heatT > 0) {
    ctx.shadowBlur = 6 + heatT * 20;
    ctx.shadowColor = `rgba(255, 240, 200, ${0.3 + heatT * 0.6})`;
  }
  ...
  if (heatT > 0) {
    ...
    const px = Math.cos(midA) * midR;
    const py = Math.sin(midA) * midR;
    let padGrad = ctx.createRadialGradient(px, py, 0, px, py, 34);
    padGrad.addColorStop(0, `rgba(255, 255, 225, ${0.55 * heatT})`);
    padGrad.addColorStop(1, 'rgba(255, 255, 225, 0)');
...
function drawDivertorSim(width) {
  ...
  for (let i = 0; i < divertorState.nodes; i++) {
    ...
    wallGrad.addColorStop(nx, rgbaStr(divertorTempColor(divertorState.T[i]), 1));
  }
  ctx.fillStyle = wallGrad;
  ctx.fillRect(startX, 10, x_Tube - startX, canvas.height * 0.75 - 20);
  ...
  divertorState.bubbles.forEach(b => {
    ...
    let bubGrad = ctx.createRadialGradient(px - b.size * 0.3, b.y - b.size * 0.3, 0.2, px, b.y, b.size);
    bubGrad.addColorStop(0, `rgba(${bubBase}, 0.9)`);
    bubGrad.addColorStop(0.7, `rgba(${bubBase}, 0.35)`);
    bubGrad.addColorStop(1, `rgba(${bubBase}, 0.05)`);
...
function drawTrussSim(width) {
  ...
  for (let i = 0; i < trussState.elements.length; i++) {
    ...
    ctx.lineWidth = 1.5 + (trussState.areas[i] / 0.001) * 3.5;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ...
    if (Math.abs(colorVal) > 0.02) {
      ...
      ctx.lineWidth = 1.2;
      ctx.setLineDash([3, 5]);
      ctx.lineDashOffset = -(time * dashSpeed) % 16;
      ctx.beginPath();
      ctx.moveTo(def_px[n1], def_py[n1]);
      ctx.lineTo(def_px[n2], def_py[n2]);
      ctx.stroke();
  ...
  for (let i = 0; i < trussState.nodes.length; i++) {
    ...
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.arc(def_px[i], def_py[i], 6, 0, Math.PI * 2);
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(def_px[i], def_py[i], 3, 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();

    ctx.fillStyle = COLORS.text;
    ctx.beginPath();
    ctx.arc(def_px[i], def_py[i], 1.5, 0, Math.PI * 2);
    ctx.fill();
...
function drawAirfoilSim(width) {
  ...
  airfoilState.particles.forEach(p => {
    ...
    ctx.strokeStyle = rgbaStr([64, 200, 210], alpha);
    ctx.lineWidth = 1.5;
    ctx.lineCap = 'round';
    ctx.beginPath();
  ...
  ctx.strokeStyle = 'rgba(70, 70, 65, 0.4)';
  ctx.lineWidth = 1;
  const bLeft = originX - sizeChord * 0.1;
  const bRight = originX + sizeChord * 1.1;
  const bTop = originY - sizeChord * 0.5;
  const bBottom = originY + sizeChord * 0.5;
  const span = (bRight - bLeft) + (bBottom - bTop);
  for (let d = -40; d < span + 40; d += 5) {
    ctx.beginPath();
    ctx.moveTo(bLeft + d, bTop);
    ctx.lineTo(bLeft + d - (bBottom - bTop), bBottom);
    ctx.stroke();
