"""
Autonomous Underwater Vehicle (AUV) Pressure Vessel Thermal Calculator
========================================================================
Steady-state and transient thermal analysis tool for a sealed electronics 
pressure vessel on an Autonomous Underwater Vehicle (AUV/UUV).

Directly useful to:
  - Sonardyne International (Yateley, Hampshire, UK) — acoustic navigation hulls
  - SEA Ltd (Bristol/Filton, UK) — naval UUV systems engineering
  - MSubs (Plymouth, UK) — small submarines and manned/unmanned underwater hulls

Physics:
  - Seawater depth-temperature profile model (thermocline modeling)
  - Internal natural convection vs Thermal Interface Material (TIM) pad conduction
  - External forced convection over a cylinder (Churchill-Bernstein correlation)
  - Transient diving cycle thermal balance (4-hour dive-transit-ascent profile)
  - External condensation (dew point) risk analysis on surfacing

References:
  - Churchill & Bernstein (1977), A correlating equation for forced convection from gases and liquids in crossflow
  - Incropera & DeWitt (2007), Fundamentals of Heat and Mass Transfer
  - UNESCO Joint Panel on Oceanographic Tables and Standards (seawater density/viscosity)

Usage:
  python auv_thermal.py             # run transient simulation and plots
  python auv_thermal.py --point     # print design point summary metrics
