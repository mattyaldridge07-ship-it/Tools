...
def churchill_bernstein_convection(V_ms, D_outer_m):
    ...
    V_eff = max(0.01, V_ms)  # clip to avoid Re=0
...
def solve_steady_state(P_elec_W, V_ms, depth_m, mat_name='titanium'):
    """steady-state junction and wall temperatures"""
...
def transient_diving_ode(t, y, P_elec_W, V_ms, mat, M_hull_kg, M_internal_kg):
    """
    AUV thermal transient during a dive cycle
    y = [T_hull_K, T_internal_K]
    """
    T_hull, T_internal = y

    # depth trajectory: dive at 1 m/s to 300m (t=0-300s), cruise (t=300-11100s), rise (t=11100-11400s)
    if t < 300.0:
        depth = 1.0 * t
    elif t < 11100.0:
        depth = 300.0
    ...
    Q_internal = (T_internal - T_hull) / R_TIM_W  # electronics through TIM
...
def plot_all(output_path):
    ...
    for i, d_ref in enumerate([10.0, 100.0, 500.0]):
        max_P = []
        for v in speeds:
            # max power such that T_junction == 85°C
            def f_P(p):
    ...
    # recompute the depth trajectory along the time vector, for the twin axis
    depth_traj = []
    for t in t_div:
        if t < 300.0:
    ...
    # from 4°C deep-water hull temp, warming in 15°C humid air
    T_air_C = 15.0
