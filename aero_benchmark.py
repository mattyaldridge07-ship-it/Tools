"""
Aerothermal Validation and Benchmarking Suite
=============================================
Validation suite verifying the 1D/2D aerothermal models of the hypersonic 
glide vehicle against published experimental, flight, and analytical data.

Directly useful to:
  - Zenotech (Bristol, UK) — FLITE3D solver benchmark validations
  - Engys (London, UK) — OpenFOAM-based CFD validation sets
  - BAE Systems Applied Intelligence (Guildford, UK) — flight simulation validation

Physics:
  - Fay & Riddell (1958) stagnation point convective heating comparison
  - Apollo 11 Command Module re-entry trajectory validation (V0=11km/s, peak q)
  - Shock tube Rankine-Hugoniot exact analytical validation (Mach 8 shock wave)
  - Flat plate radiation-equilibrium wall temperature prediction
  - PICA thermal protection system (TPS) char ablation comparison

References:
  - Fay & Riddell (1958), Theory of stagnation point heat transfer in dissociated air
  - Detra, Kemp & Riddell (1957), Addendum to heat transfer to satellite vehicles
  - NASA SP-228 (1970), Apollo 11 Mission Report
  - Anderson (2006), Hypersonic and High Temperature Gas Dynamics

Usage:
  python aero_benchmark.py             # run validations and generate plots
  python aero_benchmark.py --point     # print full markdown validation report
