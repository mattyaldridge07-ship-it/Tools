"""
2D structural truss finite element method (FEM) solver and weight optimization tool
"""
...
E_MODULUS_Pa = 70e9        # Young's modulus (Aluminium 6061-T6, 70 GPa)
...
# external loads: (NodeIdx, DOF_Direction (0=X, 1=Y), Force_N)
LOADS = [
    (6, 1, -50000.0)  # 50 kN downward at Node 6
]

# fixed boundary conditions (pinned nodes): (NodeIdx, DOF)
CONSTRAINTS = [
...
def solve_truss(areas_m2):
    """
    assembles the global stiffness matrix and solves K*U = F
    ...
    # apply boundary constraints by partitioning out the fixed DOFs
    ...
    for idx, (n1, n2) in enumerate(ELEMENTS):
        ...
        dL = (u2 - u1) * c + (v2 - v1) * s  # delta length in local coordinates
        strain = dL / L
        ...
        if force < 0:  # compression member, check buckling
            I_moment = 0.05 * (areas_m2[idx]**2)  # approximate moment of inertia for a thin-walled tube
...
def optimize_truss(max_iter=30):
    """
    minimises truss weight by resizing member cross-sectional areas
    constraints: stress safety factor > 1.5, buckling safety factor > 1.5
    """
    n_elements = len(ELEMENTS)

    areas = np.full(n_elements, 0.001)  # initial guess, 10 cm² baseline
    ...
    for iter_idx in range(max_iter):
        ...
        for idx in range(n_elements):
            ...
            buckling_demand = 0.0
            if forces[idx] < 0:
                # P_crit is proportional to Area^2, so force/P_crit scales with Area
                buckling_demand = np.sqrt(buckling[idx] * SAFETY_FACTOR)

            scale_factor = max(0.1, max(stress_demand, buckling_demand))

            new_areas[idx] = areas[idx] * (0.5 + 0.5 * scale_factor)  # damp to avoid oscillatory divergence

            new_areas[idx] = max(1e-6, new_areas[idx])  # 1 mm² floor, keeps structural connectivity
...
def plot_truss(ax, areas, U, title):
    ...
    scale_factor = 25.0  # amplify deformation so it's visible
    ...
    cmap = plt.cm.coolwarm  # red = tension, blue = compression
    ...
    for idx, (n1, n2) in enumerate(ELEMENTS):
        ...
        ax.plot(x_base, y_base, color='#222', linestyle=':', linewidth=0.8)  # undeformed reference
        ax.plot(x_def, y_def, color=color, linewidth=width)

    ax.scatter(deformed_nodes[:, 0], deformed_nodes[:, 1], color=PAPER, s=12, zorder=5)

    ax.plot([-0.05, 0.05], [-0.05, -0.05], color=GOLD, linewidth=3)  # boundary supports
