"""
SABRE-Style Precooler — Flight Trajectory Integrator
=====================================================
Extends the static envelope analysis to a time-resolved mission simulation.

Given a flight trajectory (Mach vs time, altitude vs time), integrates the
precooler heat load, LH2 consumption, frost risk, and compressor inlet
temperature as continuous time series along the actual mission profile.

Includes two representative trajectory profiles:
  1. Horizontal takeoff (INVICTUS/SABRE style): runway → Mach 5 @ 25 km
  2. Air-launched acceleration (Hermeus Chimera style): drop → mode transition

Also includes the heat exchanger geometry back-calculator:
  Given a target NTU and operating conditions, returns tube bundle geometry
  (diameter, length, number of tubes, pitch ratio) and the resulting
  pressure drop penalty.

Physics additions over sabre_precooler.py:
  - Trajectory interpolation with smooth spline fit
  - Cumulative LH2 mass consumed (integration over time)
  - Mode transition detection (turbojet → ramjet crossover)
  - Tube bundle geometry optimiser (Dittus-Boelter + Fanning friction)
  - Pressure drop correlation for compact crossflow HX

Usage:
  python trajectory_integrator.py             # full mission plots + geometry
  python trajectory_integrator.py --hermeus   # Chimera mode-transition profile
  python trajectory_integrator.py --invictus  # INVICTUS horizontal-takeoff
  python trajectory_integrator.py --geometry  # geometry back-calculator only

Author: MKA / PringlesMaths — pringlesmaths.co.uk
"""
...
def trajectory_invictus():
    """
    INVICTUS / SABRE-style horizontal takeoff to Mach 5 @ 25 km.
    Waypoints: (time_s, mach, altitude_km, humidity_gkg, phase_label)
    Based on published SABRE flight profile and Frazer-Nash INVICTUS mission.
    """
    waypoints = [
        #  t      M    alt    hum   phase
    ...
    return waypoints, 'INVICTUS Horizontal Takeoff Profile'


def trajectory_hermeus():
    """
    Hermeus Chimera TBCC mode-transition profile.
    ...
    return waypoints, 'Hermeus Chimera — Mode Transition Profile'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--invictus', action='store_true',
                        help='INVICTUS horizontal-takeoff trajectory')
    parser.add_argument('--hermeus',  action='store_true',
                        help='Hermeus Chimera mode-transition trajectory')
    ...
    run_inv  = args.invictus or args.all or not any([args.invictus, args.hermeus, args.geometry])
    run_herm = args.hermeus  or args.all or not any([args.invictus, args.hermeus, args.geometry])
    run_geo  = args.geometry or args.all or not any([args.invictus, args.hermeus, args.geometry])

    if run_inv:
        print("\n[1/3]  INVICTUS trajectory analysis...")
        wp, title = trajectory_invictus()
    ...
    if run_herm:
        print("\n[2/3]  Hermeus Chimera trajectory analysis...")
        wp, title = trajectory_hermeus()
    ...
    print("\nDone. Output files:")
    if run_inv:  print("  invictus_trajectory.png")
    if run_herm: print("  hermeus_trajectory.png")
