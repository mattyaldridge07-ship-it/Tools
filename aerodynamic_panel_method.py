...
def generate_naca_4digit(camber_pct=2, position_pct=4, thickness_pct=12, n_panels=40):
    """
    coordinates for a NACA 4-digit airfoil
    e.g. NACA 2412 -> camber_pct=2, position_pct=4, thickness_pct=12
    """
    m = camber_pct / 100.0
    p = position_pct / 10.0
    t = thickness_pct / 100.0

    # cosine spacing packs more panels near the leading/trailing edges
    ...
    # walk lower surface trailing-to-leading, then upper leading-to-trailing
...
def solve_panel_flow(x_nodes, y_nodes, alpha_deg, V_inf=1.0):
    """panel source/vortex strengths via the Hess-Smith method"""
    ...
    L = np.zeros(n_panels)
    phi = np.zeros(n_panels)  # panel angle relative to x-axis
    ...
    # (N+1)x(N+1) system: N normal-velocity boundary conditions + 1 Kutta condition
    ...
    for i in range(n_panels):
        ...
        for j in range(n_panels):
            # control point i in panel j's local frame
            dx = xc[i] - x_nodes[j]
            dy = yc[i] - y_nodes[j]
            ...
            if i == j:
                # self-panel: avoid the log/atan2 singularity
                u_source = 0.5
                v_source = 0.0
                u_vortex = 0.0
                v_vortex = -0.5
    ...
    # Kutta condition: V_t[0] + V_t[N-1] = 0, last row of A
    ...
    # lift from circulation: L = rho * V * Gamma, Gamma = sum(vortex * length)
...
def print_report():
    ...
    for alpha in [-4, 0, 4, 8, 12]:
        ...
        # thin airfoil theory, NACA 2412 zero-lift alpha ~ -2.1 deg
...
def compute_velocity_field(x_nodes, y_nodes, alpha_deg, grid_w=30, grid_h=20):
    """velocity field on a grid, for streamline plots"""
        ...
        for c in range(grid_w):
            ...
            for j in range(n_panels):
                ...
                if r1_sq < 1e-6 or r2_sq < 1e-6:
                    continue  # inside/on the airfoil boundary, skip
...
def generate_plots():
    ...
    ax2.invert_yaxis()  # Cp is conventionally plotted negative-up
