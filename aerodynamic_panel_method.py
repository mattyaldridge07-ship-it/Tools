"""
2D Potential Flow Hess-Smith Panel Method Solver for NACA Airfoils
===================================================================
Generates NACA 4-digit airfoil geometries and solves the potential flow field 
using a Source-Vorticity Panel Method. Calculates velocity vectors, surface 
pressure distributions (Cp), and integrates circulation to find the lift coefficient.

Directly useful to aerodynamics design leads at Formula 1 teams, wind turbine 
aerodynamics groups (Vestas), and aircraft manufacturers (Airbus, Boeing).

Physics:
  - NACA 4-digit profile equations (thickness, camber, chord)
  - Hess-Smith Panel Method: Source panels + single uniform vortex strength
  - Integral boundary conditions: normal velocity at control points = 0
  - Kutta condition enforced at the trailing edge (V_t1 + V_tN = 0)
  - Bernoulli pressure coefficient: Cp = 1 - (V/V_inf)^2
  - Kutta-Joukowski lift theorem: L = rho * V_inf * Gamma

Usage:
  python aerodynamic_panel_method.py             # Run default simulation and generate plots
  python aerodynamic_panel_method.py --point     # Print design point verification tables
