"""
liquid hydrogen (LH2) combustion thermodynamics, heat exchanger integration, and range sensitivity calculator
"""
...
# pull the base model in, with a standalone fallback if it's not importable
import sys, os
...
def adiabatic_flame_temp_H2(T_in_K, phi):
    """adiabatic flame temperature for hydrogen-air combustion, H2 + 0.5 O2 -> H2O, LHV = 119.96 MJ/kg"""
    FAR_stoich = 0.02915
    FAR = phi * FAR_stoich
    LHV_H2 = 119.96e6  # J/kg

    # enthalpy balance with a linear variable-Cp model, to account for dissociation at high T
    T_flame = T_in_K + phi * FAR_stoich * LHV_H2 / (CP_AIR * (1.0 + FAR))

    if T_flame > 2400.0:
        T_flame = 2400.0 + 0.3 * (T_flame - 2400.0)  # empirical dissociation cap above 2400K
    return min(3200.0, T_flame)

def solve_rayleigh_afterburner(M1, T1_K, p1_Pa, q_heat_Jkg, g=1.4):
    """1D Rayleigh flow afterburner exit Mach number and pressure drop"""
...
def solve_lh2_budget(M_flight, alt_km, phi=0.5):
    """system mass flow balance: cooling flow vs propulsion flow"""
    alt_m = alt_km * 1e3
    T_atm, p_atm, rho_atm, a_atm = isa_atmosphere(alt_m)
    T_ram, _, _, _, V_inlet = ram_conditions(M_flight, alt_m)

    # convective stagnation point heating on leading edge, Detra-Kemp-Riddell
    rho_SL = 1.225
    ...
    Thrust_req_N = MASS_VEHICLE_kg * G0 / L_D_CRUISE  # cruise thrust = drag = mg / (L/D)
    ...
    # ramjet Isp model: expand back to ambient after the Rayleigh-flow afterburner
    ...
    # total flow is whichever is larger: cooling need vs combustion burn need
    m_dot_total = np.nan
    ...
    # Breguet range: R = V * (L/D) * Isp * ln(M_wet/M_dry)
    M_wet = MASS_VEHICLE_kg
    M_dry = MASS_VEHICLE_kg - M_FUEL_CAP_kg
    range_km = np.nan
    if not np.isnan(Isp_s) and m_dot_total > 0:
        Isp_eff = Thrust_req_N / (m_dot_total * G0)  # reflects the excess fuel burned just for cooling
