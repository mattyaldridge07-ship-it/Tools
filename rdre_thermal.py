"""
rotating detonation rocket engine (RDRE) transient thermal combustor wall analysis tool
"""
...
T_LIMIT_STEADY = 1100.0     # max safe steady-state operating temperature
T_LIMIT_SHORT  = 1300.0     # short-term maximum
...
def compute_detonation_properties(prop_type='H2', phi=1.0):
    """detonation wave speed and frequency based on equivalence ratio phi"""
    prop = PROPELLANTS[prop_type]
    V_stoich = prop['V_CJ_stoich']

    V_CJ = V_stoich * (1.0 - 0.15 * (phi - 1.0)**2)  # semi-empirical fit vs phi
    f_rotation = V_CJ / (np.pi * D_ANNULUS_m)  # rotation frequency around the annulus

    return V_CJ, f_rotation

def get_instantaneous_gas_conditions(t_s, T_period_s, T_CJ_K, h_peak=100000.0, h_min=10000.0):
    """
    transient gas convective heat transfer coefficient and effective gas temp
    represents the detonation wave passing around the annulus
    """
    t_mod = t_s % T_period_s

    w_wave = 0.04 * T_period_s  # wave passage is a narrow spike, ~4% of the period

    h_gas = h_min + (h_peak - h_min) * np.exp(-((t_mod - 0.1 * T_period_s) / w_wave)**2)

    # temperature stays hot through the expansion phase then drops on refill, decays slower than h_gas
...
def solve_transient_conduction(m_dot_cool_kgs, prop_type='H2', phi=1.0, N_rotations=10):
    """
    1D heat equation, explicit, inside the Inconel wall
    boundary conditions:
    ...
    # explicit Fourier stability limit: alpha*dt/dx^2 < 0.5
    alpha_diff = K_WALL / (RHO_WALL * CP_WALL)
    stability_val = alpha_diff * dt / dx**2
    if stability_val >= 0.5:
        dt = 0.49 * dx**2 / alpha_diff  # 1e-6 is already comfortably safe, but adjust if not
        time_steps = np.arange(0.0, t_end, dt)

    # coolant forced convection, water inside the annular cooling jacket
    D_h_cool = 0.010  # hydraulic diameter
    ...
    T = np.zeros(N_NODES) + 300.0  # wall starts at coolant temperature
    ...
    for t in time_steps:
        ...
        # hot boundary (node 0): T_new[0] = T[0] + 2Fo(T1-T0) + 2Fo*dx/k*q_in
        T_new[0] = T[0] + 2.0 * Fo * (T[1] - T[0]) + 2.0 * Fo * (dx / K_WALL) * q_in

        for j in range(1, N_NODES - 1):
            T_new[j] = T[j] + Fo * (T[j-1] - 2.0 * T[j] + T[j+1])

        # cold boundary (node N-1): T_new[-1] = T[-1] + 2Fo(T[-2]-T[-1]) - 2Fo*dx/k*q_out
    ...
    # metrics over the last 3 rotation periods (quasi-steady)
    ...
    # Coffin-Manson low-cycle fatigue, Inconel coefficients C=5000, m=1.9
    C_Inconel = 5000.0
    m_fatigue = 1.9
