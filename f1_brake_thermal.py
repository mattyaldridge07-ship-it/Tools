"""
F1 Carbon-Carbon Brake Disc — Transient Thermal Model
======================================================
Predicts brake disc temperature profile during an F1 race lap as a function
of braking events, cooling duct mass flow, and disc thermal mass.

Directly relevant to:
  - F1 aerothermal departments (Cadillac F1, Williams, McLaren, Red Bull)
  - High-performance brake manufacturers (Brembo, Carbon Industrie, AP Racing)
  - Ricardo PLC (aerothermal consultancy for motorsport + aerospace)

The engineering problem:
  Carbon-carbon disc must operate in [400°C, 750°C] window:
  - Below 400°C: disc glazes, friction falls catastrophically
  - Above 750°C in air: rapid oxidation, material loss
  - Above 1000°C: catastrophic oxidation risk
  The cooling duct aerodynamicist sizes ducts to maintain this window.

Physics:
  - Lumped capacitance (single node) transient thermal model
  - Braking heat input from kinetic energy dissipation
  - Forced convection cooling (jet impingement from cooling duct)
  - Radiation to surroundings
  - Simplified oxidation kinetics (Arrhenius rate law)

Lap profiles included:
  - Silverstone GP (medium brake demand, high-speed corners)
  - Monaco GP (maximum brake demand, tight hairpins)

Known limitations:
  - Lumped capacitance (no radial/axial temperature gradient)
  - Simplified jet impingement model (real = complex internal channel)
  - No wheel rim or hub thermal coupling
  - Lap profile is idealised (real data confidential)
  - Oxidation kinetics are order-of-magnitude only

Usage:
  python f1_brake_thermal.py              # both circuits, 3 duct flows
  python f1_brake_thermal.py --silverstone
  python f1_brake_thermal.py --monaco
  python f1_brake_thermal.py --point      # steady-state table
...
# ── Disc geometry (front disc, Brembo-class F1) ───────────────────────────────
...
def plot_comparison(events_s, lt_s, events_m, lt_m, output_path):
    ...
    fig.text(0.99, 0.01, 'pringlesmaths.co.uk  |  MKA',
