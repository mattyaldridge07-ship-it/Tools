...
def cp_cc(T_C):
    """
    C-C specific heat [J/kgK] vs temperature in °C
    linear fit: ~500 J/kgK at 20°C, ~1200 J/kgK at 1200°C, ref: Buckley (1993)
    """
    return np.clip(500.0 + 0.58 * T_C, 500.0, 1200.0)

def k_cc(T_C):
    """
    C-C through-thickness thermal conductivity [W/mK], decreases with temperature (phonon scattering)
    ref: ~40 W/mK at 500°C, ~25 W/mK at 1000°C
    """
...
D_DUCT    = 0.050       # m  duct exit diameter (50mm typical motorsport duct)
...
# oxidation model: Arrhenius, dm/dt [g/m²/s] = A_ox * exp(-E_a/(R*T_K))
# fit to published C-C oxidation data (onset ~450°C, rapid above 750°C), ref: Krenkel (2008)
...
def lap_profile_silverstone():
    ...
    based on published sector time / speed data (idealised, real data is confidential)
    lap time ~1:27s, front discs carry ~70% of braking load, ~80-120 kJ per lap per disc
    """
    events = [
        (2.0,   1.8,  850),   # Turn 1, heavy braking from ~310 km/h
        (9.5,   1.2,  550),   # Turn 3, medium
        (17.0,  1.0,  480),   # Brooklands, medium
        (26.0,  1.2,  420),   # Luffield, medium-light
        (35.0,  0.8,  350),   # Village, light
        (41.0,  0.9,  400),   # Loop/Aintree, light-medium
        (55.0,  1.3,  520),   # Vale, medium, before Club
        (66.0,  0.7,  380),   # Club, light-medium, high-speed
        (72.0,  0.7,  320),   # Abbey, light
        (79.0,  1.1,  470),   # Farm, medium
        # Maggotts-Becketts-Chapel: no significant braking
        (86.0,  1.5,  600),   # Stowe, medium-heavy
    ]
...
def lap_profile_monaco():
    ...
    extreme brake demand, 12+ heavy events per lap, 280 km/h down to near-zero at hairpins
    lap time ~1:12s, brakes run very hot
    """
    events = [
        (1.5,   2.2, 1050),   # Sainte-Devote, very heavy
        (9.0,   1.8,  820),   # Massenet, medium-heavy
        (14.5,  1.5,  780),   # Casino Square, heavy
        (19.0,  2.0,  980),   # Mirabeau, very heavy
        (25.0,  1.3,  560),   # Portier, medium
        (29.0,  1.8,  850),   # Tunnel exit / Chicane, heavy
        (35.0,  1.6,  780),   # Nouvelle Chicane, heavy
        (41.0,  1.2,  520),   # Tabac, medium
        (47.0,  1.0,  460),   # Piscine S1, medium-light
        (50.0,  1.1,  500),   # Piscine S2, medium
        (55.0,  2.2, 1020),   # La Rascasse, very heavy
        (62.0,  1.6,  780),   # Anthony Noghes, heavy
    ]
...
def braking_power(t, events):
    """brake power [W] at time t given a list of braking events"""
    P = 0.0
    for (t_start, dur, P_kW) in events:
        if t_start <= t <= t_start + dur:
            tau = dur * 0.1  # smooth 10% rise/fall
...
def cooling_power(T_C, m_dot_kgs):
    ...
    turbulent jet impingement on disc face, Nu = 0.5 * Re^0.5 * Pr^0.4 (simplified Hilpert)
    ...
    Q_cool = h_jet * A_FACE * (T_C - T_AMBIENT)  # inner face only, protected from carrier
...
def oxidation_rate(T_C):
    """
    mass loss rate [g/m²/s] due to oxidation, Arrhenius model above T_ONSET
    note: gas-phase kinetics only, doesn't model oxygen diffusion through the char layer
    """
    T_K = T_C + 273.15
    if T_K < T_ONSET:
        return 0.0
...
def ode(t, y, events, m_dot_kgs):
    ...
    Q_ox   = dm_dt * 2750.0   # W, endothermic oxidation heat loss (~2750 J/g for carbon)
...
def simulate_laps(events, lap_time, m_dot_kgs, n_laps=5, T_init_C=250.0):
    """runs n_laps and returns the time-series across all of them (last lap ~ quasi-steady)"""
    T0 = T_init_C
    all_t, all_T, all_m = [], [], []
...
def lap_stats(res, lap_idx=-1):
    """stats for a given lap (default: last lap, quasi-steady)"""
    t = res['t']
    T = res['T_C']
    lt = res['lap_time']
    nl = res['n_laps']
...
def plot_circuit(circuit_name, events, lap_time, output_path):
    ...
    fig.text(0.5, 0.975,
             f'HIGH-PERFORMANCE BRAKE DISC — TRANSIENT THERMAL ANALYSIS  |  {circuit_name.upper()}',
    ...
    # FIA wear limit approx 2mm/race; thickness loss = delta_m [g/m²] / rho [g/m³]
...
def print_table(circuit_name, events, lap_time):
    """summary table across duct flows"""
    print(f"\n{'='*72}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='High-Performance Carbon-Carbon Brake Disc Thermal Model')
