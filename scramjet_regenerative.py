"""
1D marching conjugate heat transfer and structural limits solver for scramjet regenerative cooling jackets
"""
...
def hydrogen_cp(T_K):
    """specific heat [J/kgK] of hydrogen at 50 bar, ref: NIST Chemistry WebBook (approximate curve fit)"""
    if T_K < 45.0:  # peak near pseudo-critical temp, ~45 K at 50 bar
...
def eckert_gas_h(x_axial_m, M_gas, T_gas_static_K, p_gas_static_Pa):
    """supersonic gas path convective heat transfer coefficient, Eckert reference temperature, ref: Eckert (1955)"""
    T_w_guess_K = 800.0
...
def coolant_convection_h(T_cool_K, m_dot_ch_kgs):
    """heat transfer coefficient [W/m²K] inside cooling channels, Sieder-Tate for turbulent duct flow"""
...
def solve_combustor_profile(m_dot_total_kgs, M_gas=2.5, T_gas_static_K=2600.0,
    ...
    for i, x in enumerate(x_positions):
        ...
        # radial resistance chain: Q = U*Perimeter*dx*(T_recovery - T_cool), 1/U = 1/h_gas + t_wall/k_wall + 1/h_cool
        R_gas = 1.0 / h_gas
        ...
        # coolant temp update for next step, energy balance dq = m_dot*cp*dT
